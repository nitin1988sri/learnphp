...
/************************************************************************* 
Program name:  Sorting Elements of an Array by Frequency 
...
include '../Autoload.php';
 class SEAF{
    public $sortedData;
    function __construct(public Array $arr){
        $this->sortedData =oneDarraysortAlgorithms($this->arr);
    }

    function frequency(){
     
          
        $checkFreq = countArrayValuesAlgorithms($this->sortedData);
        echo "<pre>";
        print_r( $checkFreq );
        $temp=$checkFreq[array_key_first($checkFreq)];
         foreach($checkFreq as $key => $value){
           if($value > $temp){
 ...
 $obj = new SEAF([1,3,1,6,4,9,2,7,2,5,9,3,5,7,2,7,4,9,5,2]);
 $obj->frequency();